...
static struct temperature_sensor sensor_1 = {
	...
	.meas.update_interval = SENSOR_UPDATE_IVAL,
	.meas.application = 0x14, // 0x14 indoor, 0x1b inside,  0x1c outside
...
static ssize_t update_temp_read_u16(struct bt_conn *conn, const struct bt_gatt_attr *attr,
									void *buf, uint16_t len, uint16_t offset)
{
	updateSensor();
	sensor_1.temp_value = getTemperature();
...
BT_GATT_SERVICE_DEFINE(ess_svc,
					   ...
					   BT_GATT_CHARACTERISTIC(BT_UUID_TEMPERATURE,
											  ...
											  BT_GATT_PERM_READ,
											  update_temp_read_u16, NULL, &sensor_1.temp_value),
...
{
	uint8_t battery_level = 54U; // bt_bas_get_battery_level();
...
static void ess_simulate(void)
{
	...
	if (!(i % SENSOR_UPDATE_IVAL)) {
		...
		sensor_3.humid_value = (uint16_t)getHumidity();
	}
